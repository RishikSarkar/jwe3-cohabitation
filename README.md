...
- **Per-type memory**: each enclosure type keeps its own roster in the browser; sort mode and “show incompatible” persist across visits
- **Enclosure stats banner**: compatibility score and tier, animals/species counts, territory recommendation, and base appeal (muted placeholders when empty)
- **Population-aware social scoring**: adult min/max and sex-ratio limits from in-game needs lower the enclosure rating when a species is under- or over-stocked
...
- **Candidate row details**: consolidated social notes (liked by, mutual likes, dislikes, cohabitation discomfort), terrain requirements, and feeder/paleobotany deltas without duplicate lines
...
- **Show incompatible toggle**: hides social incompatibilities by default; incompatible species remain addable with dislike warnings shown in-row
...
- **Deterministic scoring**: habitat cosine similarity, shared terrain coverage, envelope tightness, feeders, cohabitation, and territory footprint
...
| `src/hooks/use-enclosure-session.ts` | URL + `localStorage` session (per-type rosters, sort, show incompatible) |
...
**Browser persistence:** Land, Aviary, and Lagoon rosters are saved separately in `localStorage` (plus sort mode and “show incompatible”). Switching enclosure type restores each list; shared `?roster=` URLs still override the matching type on load.
...
```
/?type=Land&roster=triceratops:1m2f,ankylosaurus:0m1f
```
