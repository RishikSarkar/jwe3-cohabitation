...
const DINOS = path.join(ROOT, "data", "dinosaurs.json");
...
function loadDinoRefs(): DinoRef[] {
  const data = JSON.parse(fs.readFileSync(DINOS, "utf-8")) as
    | DinoRef[]
    | { species: Array<{ id: string; name: string }> };
  if (Array.isArray(data)) return data;
  return data.species.map((s) => ({ id: s.id, name: s.name }));
}

async function main() {
  const dinos = loadDinoRefs();
